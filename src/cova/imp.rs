...
use gst::glib;
use gst::prelude::*;
use gst::{gst_debug, gst_info, gst_trace, gst_log, ClockTime, BufferFlags};
...
struct State {
    ...
    bufs: LinkedList<(ClockTime, ClockTime, LinkedList<Buffer>, LinkedList<Buffer>, bool)>,
...
impl Cova {
    ...
    fn sink_mask_chain(
        ...
        let tracker = state.tracker.get_or_insert_with(|| {
            ...
            Tracker::new(
                ...
                (*settings).cc_threshold as u32,
                false
            )
        });

        let pts = buffer.pts().unwrap();
        gst_trace!(CAT, obj: element, "Buffer Recived, PTS: {}", pts);

        let bufref = buffer.make_mut();
        let mut map = bufref.map_writable().unwrap();
        let min_required = tracker.update(&mut map.as_mut_slice()[..45*80], pts.nseconds());
                ...
                .bufs
                    ...
                    .filter(|(min, max, _, _, _)| &min_track_pts <= max && min <= &max_track_pts) {
                        ...
                        gst_log!(
                            ...
                            min, max, decoded_dependency + decoded_inference, decoded_inference);
                    ...
                    .iter_mut()
                        ...
                        .for_each(|(_, _, ref mut gop, ref mut out, _)| {
                            ...
                            for _ in 0..remainder {
                                ...
                                buf.get_mut().unwrap().set_flags(gst::BufferFlags::DROPPABLE);
                                decoded_dependency += 1;
                                out.push_back(buf);
                            }

                            for _ in 0..extra_infer {
                                let num_dependant = step_extra_infer.checked_sub(1).unwrap_or(0);
        ...
        let droppable_gops = state.bufs.drain_filter(|(_, max, _, _, finalized)| *finalized && max <= &mut droppable_pts);
    ...
    fn sink_enc_chain(
        ...
        if !buffer.flags().contains(BufferFlags::DELTA_UNIT) {
            ...
            gst_trace!(CAT, obj: element, "IDR frame encoundtered at [PTS: {}]", pts);
            state.bufs.push_back((pts, pts, new_gop_list, LinkedList::new(), false));
        } else {
    ...
    fn src_event(
        ...
        let mut ret = true;
        if let gst::EventView::Eos(eos_event) = event.view() {
            gst_info!(CAT, obj: element, "EOS recieved from src");
            if eos_event.is_upstream() {
                ret = self.sink_mask_pad.push_event(event.copy());
            }
        }
        ret && self.sink_enc_pad.push_event(event)
    }
    fn src_query(
        &self,
        _pad: &gst::Pad,
...
impl ObjectSubclass for Cova {
    ...
    fn with_class(klass: &Self::Class) -> Self {
        ...
        let src_pad = gst::Pad::builder_with_template(&klass.pad_template("src").unwrap(), Some("src"))
            .event_function(|pad, parent, event| {
                Cova::catch_panic_pad_function(
                    parent,
                    || false,
...
impl ObjectImpl for Cova {
    ...
    fn set_property(
        ...
        match pspec.name() {
            ...
            "port" => {
                ...
                gst_info!(CAT, obj: obj, "Changing port from {} to {}", settings.port, port);
                settings.port = port;
            }
            "infer-i" => {
                let mut settings = self.settings.lock().unwrap();
    ...
    fn dispose(&self, _obj: &Self::Type) {
        let counts = self.counts.lock().unwrap();
        println!("decoded_inference:{},decoded_dependency:{},dropped:{}", counts.decoded_inference, counts.decoded_dependency, counts.dropped);
    }
}
