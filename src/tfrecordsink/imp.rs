...
impl TFRecordSink {
    #[inline]
    fn parse_bytes_into_example(&self, in_frame: gst_video::VideoFrameRef<&gst::BufferRef>)
        -> Result<(), gst::ErrorMessage>
    {
        ...
        let mut example =
            match self.example.lock().unwrap().take() {
                Some(example) => example,
                None => {
                    let settings = self.settings.lock().unwrap();
                    TFRecordSink::new_example(settings.gop as usize)
                }
            };
        ...
        for chunk in map.chunks_exact(stride).flat_map(|l| l[..line_bytes].chunks_exact(4)) {
            vec_mb_type.push(chunk[0]);
            vec_mv_x.push(chunk[1]);
            vec_mv_y.push(chunk[2]);
        }

        cast!(example.get_mut("mb_type").unwrap(), Feature::BytesList).push(vec_mb_type);
        cast!(example.get_mut("mv_x").unwrap(), Feature::BytesList).push(vec_mv_x);
        cast!(example.get_mut("mv_y").unwrap(), Feature::BytesList).push(vec_mv_y);


        unsafe { vec_gt.set_len(vec_gt.capacity()); }
        ...
        cast!(example.get_mut("gt").unwrap(), Feature::BytesList).push(vec_gt);

        *self.example.lock().unwrap() = Some(example);
    ...
    fn write(&self) -> Result<(), tfrecord::Error> {
        ...
        let mut example = { self.example.lock().unwrap().take().unwrap() };

        let gop = {
            let settings = self.settings.lock().unwrap();
            settings.gop as usize
        };
    ...
    fn zero_fill_example(example: &mut Example, count: usize, size: usize) {
        {
            let v = cast!(example.get_mut("mb_type").unwrap(), Feature::BytesList);
            (0..count).for_each(|_| v.push(vec![0; size]));
        }
        {
            let v = cast!(example.get_mut("mv_x").unwrap(), Feature::BytesList);
            (0..count).for_each(|_| v.push(vec![0; size]));
        }
        {
            let v = cast!(example.get_mut("mv_y").unwrap(), Feature::BytesList);
            (0..count).for_each(|_| v.push(vec![0; size]));
        }
        {
            let v = cast!(example.get_mut("gt").unwrap(), Feature::BytesList);
            (0..count).for_each(|_| v.push(vec![0; size]));
        }
    }

    fn new_example(size: usize) -> Example {
        Example::from_iter([
            (
                "mb_type".into(),
                Feature::BytesList(Vec::with_capacity(size)),
            ),
            (
                "mv_x".into(),
                Feature::BytesList(Vec::with_capacity(size)),
...
impl BaseSinkImpl for TFRecordSink {
    ...
    fn start(&self, element: &Self::Type) -> Result<(), gst::ErrorMessage> {
        ...
        let writer: ExampleWriter<_> = RecordWriterInit::create(location).map_err(|err| {
            gst::error_msg!(
    ...
    fn render(
        ...
        let in_frame = gst_video::VideoFrameRef::from_buffer_ref_readable(
            buffer.as_ref(), &video_info).map_err(|_| {
            gst::element_error!(
                element,
                gst::CoreError::Failed,
