        ...
        ! h264parse config-interval=-1 ! queue max-size-buffers=0 max-size-bytes=0 max-size-time=0 \
            ...
            t0.src_0 \
                ...
                ! c.sink_enc cova name=c \
                    cc-threshold=3 sort-maxage=30 sort-iou=0.5 sort-minhits=50 \
            ...
            t0.src_1 \
                ...
                ! m_fcn0.sink_0 nvstreammux name=m_fcn0 width=80 height=180 batch-size=512 \
                    buffer-pool-size=1024 nvbuf-memory-type=2 \
                ! nvinfer config-file-path=/workspace/deepstream/config/fcn/vldb/archie_b512.txt \
                ! queue max-size-buffers=1024 max-size-bytes=0 max-size-time=0 \
